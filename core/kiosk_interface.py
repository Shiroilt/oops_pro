...
class KioskInterface:
    ...
    def __init__(self, kiosk: Kiosk):
        # IMPORTANT:
        # Keep attribute name '_kiosk' unchanged because
        # other project files directly access it.
        self._kiosk = kiosk
    ...
    def purchase_item(self, item_name: str, user_id: str,
        ...
        product = self._kiosk._inventory.find_item(item_name)
        ...
        if success_count > 0:
            ...
            if remaining <= 5:
                print(f"\n  *** WARNING: Only {remaining} unit(s) left for "
                      f"'{item_name}' ***")
    ...
    def refund_transaction(self, txn_id: str, user_id: str) -> bool:
        ...
        if txn:
            product = self._kiosk._inventory.find_item(
                txn.get("item", "")
            )
    ...
    def restock_inventory(self, item_name: str,
                          quantity: int,
        ...
        if result:
            ...
            if (
                self._kiosk._emergency_mode and
    ...
    def run_diagnostics(self) -> dict:
        ...
        hardware = self._kiosk._hardware
        ...
        report = hardware.run_diagnostics()

        if self._kiosk._sensors:
            report["sensors"] = self._kiosk._sensors.run_diagnostics()
    ...
    def _persist(self):
        """Persist kiosk state after successful operations."""
